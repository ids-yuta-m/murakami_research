...
This model takes compressed measurement (1 channel) + mask (16 channels) as input
...
class DecisionNet(nn.Module):
    def __init__(self, num_classes=16):
        ...
        # Input: compressed measurement (1 channel) + mask (16 channels) = 17 channels total
    ...
    def forward(self, compressed_measurement, mask, return_logits=False):
        """
        Args:
        ...
        x = torch.cat([compressed_measurement, mask], dim=1)  # [B, 17, 8, 8]
    ...
    def get_logits(self, compressed_measurement, mask):
        """Get raw logits (for training with proper loss functions)"""
        return self.forward(compressed_measurement, mask, return_logits=True)

    def get_probabilities(self, compressed_measurement, mask):
        """Get probability distribution (for inference/evaluation)"""
        return self.forward(compressed_measurement, mask, return_logits=False)
...
class DecisionNetWithEpsilonGreedy(DecisionNet):
    ...
    def forward_with_exploration(self, compressed_measurement, mask, training=True):
        """
        ...
        model_probs = super().forward(compressed_measurement, mask, return_logits=False)
