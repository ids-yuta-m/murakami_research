"""
Training script for mask selection model using a randomly sampled mask's
compressed image as input.

Based on train_decision_id_8x8_single_mask.py.
At each training step one mask is randomly drawn from the 16 candidates to
compress the current block.  The model learns to predict the optimal mask
from any compressed measurement.

Key differences from train_decision_id_8x8_single_mask.py:
- Input mask is randomly sampled from all 16 masks at each step (instead of
  a single fixed mask).
- Evaluation averages logits over all 16 possible compressions.

Input Format:
- Compressed measurement: [B, 1, 8, 8] - sum of a randomly sampled mask
...
def evaluate_selection(decision_net, masks, psnr_npz_path, test_video_dir,
    ...
    For each test sample the model receives the compressed measurement from
    each of the 16 masks; the logits are averaged before taking argmax.
    This gives a deterministic, mask-agnostic evaluation.
    """
    ...
    if len(test_samples) > num_samples:
        ...
        indices = rng.choice(len(test_samples), size=num_samples, replace=False)
    ...
    with torch.no_grad():
        for sample in test_samples:
            video = sample['video']
            patch = video[sample['start_frame']:sample['start_frame'] + 16,
                         sample['start_h']:sample['start_h'] + 8,
                         sample['start_w']:sample['start_w'] + 8]
            if patch.max() > 1.0:
                patch = patch / 255.0

            gt_block = torch.from_numpy(patch).float().unsqueeze(0).to(device)

            # Average logits over all 16 masks for deterministic evaluation
            avg_logits = torch.zeros(1, 16, device=device)
            for mask in masks:
                compressed = torch.sum(gt_block * mask.unsqueeze(0), dim=1, keepdim=True)
                avg_logits += decision_net.get_logits(compressed)
            avg_logits /= len(masks)

            predicted_idx = torch.argmax(avg_logits, dim=1).item()
            best_mask_idx = int(np.argmax(psnr_results[sample['key']]))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, sequence_metadata in enumerate(sequence_loader):
            ...
            for block_meta in sequence:
                ...
                # Randomly sample ONE mask for compression
                ...
                # Forward pass
                logits = decision_net.get_logits(compressed)
